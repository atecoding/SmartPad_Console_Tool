...
extern int command_print(const char *format, ...);
...
int eval() {
    ...
    while (1) {
        ...
        else if (op == READ) { ax = fread((char *)sp[3], (int)sp[2], (int)sp[1], (FILE *)*sp); }
        else if (op == PRTF) { tmp = sp + pc[1]; ax = command_print((char *)tmp[-1], tmp[-2], tmp[-3], tmp[-4], tmp[-5], tmp[-6]); }
...
int complie_file(int argc, char **argv)
{
    int i;
    int *tmp;
	FILE * fp = 0;
